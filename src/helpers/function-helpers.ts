...
export const generateUniqueEndpoint = async (title: string, model: 'blog' | 'landing', currentModelId?: string): Promise<string> => {
  let slugifiedTitle = '';
  ...
  while (!isUnique) {
    if (model === 'blog') {
      ...
      const [existingPage, existingBlogCategory] = await Promise.all([
